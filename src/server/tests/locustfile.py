...
class GameSimulationUser(HttpUser):
    ...
    def interact_with_games(self):
        ...
        if self.game_id is not None:  # Check if game_id is set
            ...
            self.client.post(f'/api/join_game/{self.game_id}', json={
                'password': 'value', 
                'user': {'id': '789', 'name': 'Black Widow'}
            }, name="/api/join_game/[game_id]", headers=headers)
            self.client.post(f'/api/game/{self.game_id}', json={
                'old_pos': '0,0', 
                'new_pos': '0,1'
            }, name="/api/game/[game_id]", headers=headers)
