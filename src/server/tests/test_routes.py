...
def test_new_game(client: FlaskClient):
    response = client.post('/api/new_game', json={'password': 'value', 'color': 'black', 'user': {'id': '123', 'name': 'Captain America'}})
...
def test_join_game_not_found(client: FlaskClient):
    response = client.post('/api/join_game/999999', json={'password': 'value', 'user': {'id': '123', 'name': 'Captain America'}})
...
def test_join_game_already_in_game(client: FlaskClient):
    response = client.post(f'/api/join_game/{game_id}', json={'password': 'value', 'user': {'id': '123', 'name': 'Captain America'}})
    assert response.status_code == 200
    assert response.get_json() == {'game_id': game_id, 'message': 'User is already in the game.'}

def test_join_game_incorrect_password(client: FlaskClient):
    response = client.post(f'/api/join_game/{game_id}', json={'password': 'wrong', 'user': {'id': '123', 'name': 'Captain America'}})
    assert response.status_code == 401
    assert response.get_json() == {'error_message': 'Incorrect game password. Access denied.', 'status': 401}

def test_join_game_success(client: FlaskClient):
    response = client.post(f'/api/join_game/{game_id}', json={'password': 'value', 'user': {'id': '789', 'name': 'Iron Man'}})
    assert response.status_code == 200
    assert response.get_json() == {'game_id': game_id, 'status': 200}

def test_join_game_full(client: FlaskClient):
    response = client.post(f'/api/join_game/{game_id}', json={'password': 'value', 'user': {'id': '456', 'name': 'Spiderman'}})
    assert response.status_code == 409
    assert response.get_json() == {'error_message': 'The game is full. Cannot join.', 'status': 409}
...
def test_game_post_move_piece(client: FlaskClient):
    response = client.post(f'/api/game/{game_id}', json={'old_pos': '0,0', 'new_pos': '0,1'})
    assert response.status_code == 200
...
def test_repeated_calls(client: FlaskClient):
    ...
    # New game
    response3 = client.post('/api/new_game', json={'password': 'value', 'color': 'black', 'user': {'id': '123', 'name': 'Captain America'}})
    response4 = client.post('/api/new_game', json={'password': 'value', 'color': 'black', 'user': {'id': '123', 'name': 'Captain America'}})
    assert response3.status_code == 200
    assert response3.get_json()['game_id'] != response4.get_json()['game_id']

    # Join game
    response5 = client.post(f'/api/join_game/{game_id}', json={'password': 'value', 'user': {'id': '123', 'name': 'Captain America'}})
    response6 = client.post(f'/api/join_game/{game_id}', json={'password': 'value', 'user': {'id': '123', 'name': 'Captain America'}})
...
def test_game_post_move_piece_correct_game(client: FlaskClient):
    ...
    response = client.post(f'/api/game/{game_id}', json={'old_pos': '0,1', 'new_pos': '0,0'})
    data = response.get_json()
    board = data['board']
